...
test('open file picker', async ({ page }) => {
  const fsaMockPath = path.resolve('node_modules', 'fsa-mock', 'dist', 'fsa-mock.umd.cjs')
  ...
  await expect(page.frameLocator('[name="editor-canvas"]').getByRole('document', { name: 'Empty block' })).toBeFocused()
})
