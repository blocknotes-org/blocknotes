...
@objc public class Filesystem: NSObject {
    ...
    @objc public func getFileUrl(at path: String, in directory: String?) -> URL? {
        if directory == "ICLOUD" {
            ...
            if let ubiquityContainerURL = fileManager.url(forUbiquityContainerIdentifier: nil) {
                ...
                return iCloudDocumentsURL.appendingPathComponent(path)
            }
            return nil
        } else if let directory = directory, let bookmarkData = Data(base64Encoded: directory) {
            var isStale = false
            // Attempt to resolve the bookmark data to a URL
            do {
                let url = try URL(resolvingBookmarkData: bookmarkData, bookmarkDataIsStale: &isStale)
                // Handle stale bookmarks if needed
                if !isStale {
        ...
        } else if let directory = getDirectory(directory: directory) {
